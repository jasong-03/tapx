...
const _formSchema = z.discriminatedUnion("type", [
  depositFormSchema,
  withdrawFormSchema,
])
...
export const DepositWithdrawForm = ({ ammPackageId, baseAssetType, quoteAssetType, lpTokenType, vaultId, priceIds }: DepositWithdrawFormProps) => {
  ...
  const { data: baseAssetMetadata } = useQuery({
    queryKey: ["coin-metadata", baseAssetType],
    queryFn: () => client.getCoinMetadata({ coinType: baseAssetType }),
    enabled: !!baseAssetType
  });
  const { data: quoteAssetMetadata } = useQuery({
    queryKey: ["coin-metadata", quoteAssetType],
    queryFn: () => client.getCoinMetadata({ coinType: quoteAssetType }),
    enabled: !!quoteAssetType
  });
  const { data: lpTokenMetadata } = useQuery({
    queryKey: ["coin-metadata", lpTokenType],
    queryFn: () => client.getCoinMetadata({ coinType: lpTokenType }),
    enabled: !!lpTokenType
  });

  const { data: priceData } = useTokenPrices(ammPackageId, priceInfoObjectIds || []);
