// Pyth oracle price refresh for DeepBook margin operations (engine version).
// Ported from web/src/lib/deepbook/pyth-refresh.ts

import { bcs } from '@mysten/sui/bcs';
import { Transaction, coinWithBalance } from '@mysten/sui/transactions';
import { getNetwork, type SuiNetwork } from './pools.js';

const MAX_ARGUMENT_SIZE = 16 * 1024;

// ── Pyth VAA cache: background refresh, reuse for trades ──
const PYTH_CACHE_TTL_MS = 5_000; // 5s cache TTL
const PYTH_REFRESH_INTERVAL_MS = 3_000; // refresh every 3s

interface PythCacheEntry {
  updates: Uint8Array[];
  fetchedAt: number;
}

const pythVaaCache = new Map<string, PythCacheEntry>();
const pythRefreshTimers = new Map<string, ReturnType<typeof setInterval>>();

export async function warmPythCache(poolKey: string): Promise<void> {
  const feedInfo = getFeedInfoForPool(poolKey);
  if (feedInfo.length === 0) return;

  const cacheKey = feedInfo.map(f => f.feedId).sort().join(',');

  // Initial fetch
  try {
    const feedIds = feedInfo.map(f => f.feedId);
    const updates = await fetchPriceUpdateData(feedIds);
    pythVaaCache.set(cacheKey, { updates, fetchedAt: Date.now() });
  } catch (err) {
    console.warn(`[PythCache] Initial fetch for ${poolKey} failed:`, err);
  }

  // Background refresh
  if (!pythRefreshTimers.has(cacheKey)) {
    ...
    }, PYTH_REFRESH_INTERVAL_MS);
    pythRefreshTimers.set(cacheKey, timer);
  }
}

export function getCachedPythVaa(poolKey: string): Uint8Array[] | null {
  const feedInfo = getFeedInfoForPool(poolKey);
  if (feedInfo.length === 0) return null;

  const cacheKey = feedInfo.map(f => f.feedId).sort().join(',');
  const entry = pythVaaCache.get(cacheKey);
  if (!entry) return null;
  if (Date.now() - entry.fetchedAt > PYTH_CACHE_TTL_MS) return null;
...
const PYTH_CONFIGS: Record<SuiNetwork, {
  pythStateId: string;
...
const PRICE_FEEDS: Record<SuiNetwork, Record<string, { feedId: string; priceInfoObjectId: string }>> = {
  testnet: {
    DEEP: {
...
async function fetchPriceUpdateData(feedIds: string[]): Promise<Uint8Array[]> {
  ...
  const cfg = PYTH_CONFIGS[network];
  const resp = await fetch(`${cfg.hermesUrl}/v2/updates/price/latest?${params.toString()}`);
  if (!resp.ok) {
...
function getFeedInfoForPool(poolKey: string): Array<{ feedId: string; priceInfoObjectId: string }> {
  ...
  const network = getNetwork();
  const feeds = PRICE_FEEDS[network];
...
async function addPythPriceUpdates(tx: Transaction, poolKey: string): Promise<string[]> {
  const feedInfo = getFeedInfoForPool(poolKey);
  if (feedInfo.length === 0) return [];
  ...
  // Use cached VAA if available, otherwise fetch fresh
  const cached = getCachedPythVaa(poolKey);
  const updates = cached ?? await fetchPriceUpdateData(feedIds);
  ...
  const [verifiedVaa] = tx.moveCall({
    target: `${cfg.wormholePackageId}::vaa::parse_and_verify`,
    arguments: [
      tx.object(cfg.wormholeStateId),
  ...
  let [priceUpdatesHotPotato] = tx.moveCall({
    target: `${cfg.pythPackageId}::pyth::create_authenticated_price_infos_using_accumulator`,
    arguments: [
      tx.object(cfg.pythStateId),
      tx.pure(
        bcs
          .vector(bcs.U8)
  ...
  for (const feed of feedInfo) {
    ...
    [priceUpdatesHotPotato] = tx.moveCall({
      target: `${cfg.pythPackageId}::pyth::update_single_price_feed`,
      arguments: [
        tx.object(cfg.pythStateId),
        priceUpdatesHotPotato,
        tx.object(feed.priceInfoObjectId),
        coinWithBalance({ balance: cfg.baseUpdateFee }),
        tx.object.clock(),
      ],
    });
  }
  ...
  tx.moveCall({
    target: `${cfg.pythPackageId}::hot_potato_vector::destroy`,
    arguments: [priceUpdatesHotPotato],
    typeArguments: [`${cfg.pythPackageId}::price_info::PriceInfo`],
  });
